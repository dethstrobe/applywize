...
const createApplication = async (page: Page) => {
  ...
  await page.getByRole("textbox", { name: "Email" }).fill("we.coyote@acme.com")
  ...
  await applicationForm.getByRole("button", { name: "Create" }).click()
...
test.describe
  .serial(
    withDocCategory("Applications Flow", {
      label: "Applications",
      position: 2,
    }),
    () => {
      test.describe(
        withDocCategory("Create Application Page", {
          label: "Create Application",
          position: 1,
        }),
        () => {
          test.describe(
            withDocMeta("Create a new application", {
              sidebar_position: 1,
            }),
            () => {
          ...
          test.describe(
            withDocMeta("Removing a contact from a new application", {
              sidebar_position: 2,
            }),
            () => {
              test("Removing a contact from a new application", async ({
                ...
                await test.step("clicking the delete button will remove the contact from the application", async () => {
                  await page
                    ...
                    .click()
                  const contactList = page.getByRole("list", {
                    name: "Contacts",
                  })
      ...
      test.describe(
        withDocMeta("Applications Dashboard", {
          sidebar_position: 2,
        }),
        () => {
          test("Logo should redirect to home page", async ({
            ...
            await test.step("when the logo is clicked.", async () => {
            ...
            await test.step("It should navigate to home page.", async () => {
      ...
      test.describe(
        ...
        () => {
          ...
          test.describe(
            ...
            () => {
              test("To edit an existing application", async ({
                page,
              }, testInfo) => {
          ...
          test.describe(
            withDocMeta("Cancel an existing application", {
              sidebar_position: 2,
            }),
            () => {
              test("To cancel an in-progress application", async ({
                page,
              }, testInfo) => {
                const { companyName } = await createApplication(page)
          ...
          test.describe(
            withDocMeta("Delete an application", {
              sidebar_position: 3,
            }),
            () => {
              test("To delete an application", async ({ page }, testInfo) => {
                const { companyName } = await createApplication(page)
